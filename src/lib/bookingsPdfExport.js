...
function routeLine(b) {
  ...
  // ── 2026-09-20: mỗi hành trình 1 dòng, dùng em dash "—", KHÔNG hiển thị giờ bay ──
  return segs.map(s => ({
...
function feesLines(t, currency) {
  ...
  // ── 2026-09-20: mỗi dòng phí = 2 cột (nhãn trái, số tiền phải) ──
  return fees.map(f => ({
    columns: [
      {
...
function kindLine(b) {
  // ── 2026-09-20: 3 dòng — Loại, Hãng, Ngày bán (dd/MM/yy) ──
  const lines = [
...
function buildRows(bookings) {
  ...
  for (const b of bookings) {
    // ── 2026-09-20: sort tickets giống UI (group theo companyId) ──
    ...
    // ── 2026-09-20 gộp: Tổng booking + Quy đổi VND vào 1 cell ──
    // - VND: chỉ 1 dòng tổng.
    // - USD: dòng 1 = tổng USD, dòng 2 = quy đổi VND + tỷ giá.
    const totalStack = [
      { text: fmt(total, b.currency), bold: true, fontSize: 8, alignment: 'right' },
    ]
    if (b.currency === 'USD') {
      const rate = parseAmount(b.exchangeRate) || 0
      const vnd = total * rate
      totalStack.push(
        { text: '≈ ' + formatMoney(vnd, 'VND', { withUnit: false }), fontSize: 7.5, color: '#0369a1', alignment: 'right', bold: true },
        { text: 'TG ' + formatMoney(rate, 'VND', { withUnit: false }), fontSize: 6.5, color: '#666', alignment: 'right' },
      )
    }
    ...
    tickets.forEach((t, i) => {
      ...
      const numCell = (v) => cell(
        { text: fmt(v, b.currency), fontSize: 8, alignment: 'right' },
        { alignment: 'right' }
      )
      ...
      // Booking code — rowSpan
      if (isFirst) {
        row.push(cell(
          [
            { text: b.bookingCode || '—', bold: true, fontSize: 8 },
            { text: b.currency, fontSize: 6.5, color: '#666' },
          ],
          { rowSpan: N }
        ))
      } else {
      ...
      row.push(cell(feesLines(t, b.currency), { alignment: 'left' }))
...
export async function exportBookingsReport(filterParams, { onProgress } = {}) {
  ...
  // ── 2026-09-20: KHÔNG sort — giữ nguyên thứ tự API, khớp UI ──
  ...
  const dateRange = fromMs && toMs
    ? `từ ngày ${fmtDate(fromMs)} đến ngày ${fmtDate(toMs)}`
    : (fromMs ? `từ ngày ${fmtDate(fromMs)}` : (toMs ? `đến ngày ${fmtDate(toMs)}` : '(toàn bộ)'))

  const body = [tableHeader(), ...buildRows(bookings)]

  // ── 2026-09-20 điều chỉnh widths ────────────────────────────
  // - Mã BK: 45 → 32 (mã booking 5-6 ký tự thôi, ko cần rộng vậy)
  // - Các loại phí: 92 → 64 (-30%), pdfmake tự wrap trong ô
  // - Note: '*' nên tự nhận toàn bộ 41pt được giải phóng
  const widths = [
    38,   // A Kind
    68,   // B Customer
    32,   // C Booking  (giảm)
  ...
  const docDefinition = {
    ...
    header: {
      margin: [20, 18, 20, 0],
      stack: [
        { text: 'BÁO CÁO CÁC VÉ ĐÃ BÁN', fontSize: 14, bold: true, alignment: 'center' },
